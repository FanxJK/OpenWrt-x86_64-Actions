...
function getCloudVer() {
	...
	github=(${github//// })
	curl "https://api.github.com/repos/${github[2]}/${github[3]}/releases/latest" | jsonfilter -e '@.tag_name' | sed -n 's/^v\([0-9]\+\.[0-9]\+\(\.[0-9]\+\)*\).*/\1/p'
}
...
function updateCloud() {
	...
	lFirVer=$(cat /etc/openwrt_release | sed -n "s/DISTRIB_RELEASE='\(.*\)'/\1/p")
